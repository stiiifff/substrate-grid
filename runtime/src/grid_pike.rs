...
decl_storage! {
	trait Store for Module<T: Trait> as GridPike {
		Organizations get(organizations): map H256 => Organization;
	}
}
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn create_org(origin, id: Vec<u8>, name: Vec<u8>) -> Result {
			...
			ensure!(id.len() > 0 && value.len() <= BYTEARRAY_LIMIT, format!("Organization ID required (1-{} bytes)", BYTEARRAY_LIMIT));
			ensure!(name.len() > 0 && value.len() <= BYTEARRAY_LIMIT, format!("Organization name required (1-{} bytes)", BYTEARRAY_LIMIT));

			let digest = <<T as system::Trait>::Hashing as Hash>::hash(&id);
			ensure!(Organizations::get(digest) == None, format!("Organization already exists: {}", id.to_string()));
